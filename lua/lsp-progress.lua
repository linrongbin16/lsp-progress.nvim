  ...
  if self.console then
    ...
    for _, m in ipairs(split_msg) do
      vim.cmd(
        string.format(
          [[echom "%s"]],
          vim.fn.escape(log_format(self.counter, m), '"')
        )
      )
    end
...
local function spin(client_id, token)
  ...
  if series.done then
    local function remove_series_later()
      ...
      LOGGER:debug(
        "Series removed (client_id:" .. client_id .. ",token:" .. token .. ")"
      )
...
local function progress_handler(err, msg, ctx)
  ...
  elseif value.kind == "report" then
    ...
    if series then
      ...
      LOGGER:debug(
        "Series not found when update: client_id:"
          .. client_id
          .. ", token:"
          .. token
      )
    ...
    if client:hasSeries(token) then
      ...
      LOGGER:debug(
        "Series not found when end: client_id:"
          .. client_id
          .. ", token:"
          .. token
      )
