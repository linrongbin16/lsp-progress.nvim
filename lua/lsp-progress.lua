...
local function emit_event()
    if not Emited then
        ...
        vim.defer_fn(reset_emited, Config.event_update_time_limit)
...
local function progress_handler(err, msg, ctx)
    ...
    elseif value.kind == "report" then
        ...
        if value.kind ~= "end" then
            logger.warn("Unknown message kind `%s` from client %d-%s", value.kind, client_id, client_name)
        end
    ...
    for client_id, client_data in pairs(Clients) do
        if vim.lsp.client_is_stopped(client_id) then
            ...
            for token, series in pairs(client_data.serieses) do
                local fmtkey = series:format_key()
                if deduped_serieses[fmtkey] then
                    ...
                    local old_series = deduped_serieses[fmtkey]
                    if series.percentage == nil then
                        deduped_serieses[fmtkey] = series
                        logger.debug(
                    ...
                    elseif old_series.percentage ~= nil then
                        ...
                        deduped_serieses[fmtkey] = series
                            and series.percentage < old_series.percentage
                            or old_series
                        logger.debug(
